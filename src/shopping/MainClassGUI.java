...
public class MainClassGUI extends JFrame {
 ...
 if (currentUser != null) {
	    Logger logger = FileLogger.getInstance(); // Retrieve the singleton instance
 ...
 List<ProductFactory> productList = catalog.getAllProducts();
 ...
 for (ProductFactory product : productList) {
 ...
 List<ProductFactory> cartItems = shoppingCart.getItems();
 ...
 ProductFactory productToRemove = findProductByName(cartItems, productName);
 ...
 java.util.List<ProductFactory> cartItems = shoppingCart.getItems();
 ...
 for (ProductFactory item : cartItems) {
 ...
 JOptionPane.showMessageDialog(null, "Your shopping cart is empty.", "Shopping Cart", JOptionPane.INFORMATION_MESSAGE);
	...
	private void placeOrder() {
	    if (currentUser != null) {
	        ...
	        String[] paymentOptions = {"Credit Card", "Debit Card", "UPI", "Cash"};
	        ...
	        if (selectedPaymentMethod != null) {
	        	...
	        	PaymentProcessor paymentProcessor = new PaymentProcessor(FileLogger.getInstance()); // Initialize payment processor
	        	boolean paymentSuccess = paymentProcessor.processPayment(totalPrice, selectedPaymentMethod);
	        	if (paymentSuccess) {
	        	    Logger logger = FileLogger.getInstance(); // Retrieve the singleton instance of FileLogger
	        	    ...
	        	    shoppingCart.clearCart(); // Clear the cart after successful payment
	        	} else {
	        	    Logger logger = FileLogger.getInstance(); // Retrieve the singleton instance of FileLogger
	            ...
	            JOptionPane.showMessageDialog(null, "Payment method selection cancelled.");
	        }
	    } else {
